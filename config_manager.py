"""
This module manages the configuration settings for the GeminiSH application.
"""
...
class ConfigManager:
    """
    Manages the configuration settings for the GeminiSH application.

    Attributes:
        DEFAULT_DIR (str): The default directory for configuration files.
    ...
    def __init__(self):
        ...
        self.config = (
            self.load_config(os.path.join(self.default_directory, self.CONFIG_FILE), True) or {}
        )
        ...
        if self.default_directory != self.agent_directory:
            self.config_agent = self.load_config(
                os.path.join(self.agent_directory, self.CONFIG_FILE)
            )
    ...
    def load_config(self, file_path, raise_error=False):
        ...
        if os.path.exists(file_path):
            ...
            raise FileNotFoundError(
                f"The configuration file: {file_path} was not found and therefore cannot continue"
            )
        else:
            return None
